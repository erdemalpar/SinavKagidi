...
def update_preview_html():
    file_path = os.path.join(BASE_DIR, 'templates/sinav_onizleme.html')
...
<style>
    @media print {
        ...
        nav, aside, header, footer, .no-print, #sidebar, #menuToggle {
        ...
        .sinav-kagidi {
            ...
            padding: 0 !important;
            width: 100% !important;
            max-width: 210mm !important; /* A4 Genişliği */
        }
        
        .a4-sayfa {
            padding: 2.5cm 2cm 2cm 2cm !important; /* Standart kenar boşlukları */
            width: 100% !important;
            margin: 0 !important;
        ...
        /* Sayfa sonu ayarları */
        .soru-blok {
            break-inside: avoid; /* Soruyu bölme */
...
<div class="space-y-8 pb-20">
    <!-- Kontrol Paneli -->
    <div class="no-print bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sticky top-4 z-40 flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">{{ sinav.sorular|length }} Soru • {{ sinav.sorular|sum(attribute='soru.puan') }} Puan</p>
            </div>
        </div>
            ...
            <header class="border-b-4 border-slate-900 pb-6 mb-8 text-center">
                ...
                <div class="mt-8 grid grid-cols-2 gap-x-12 gap-y-4 text-left text-sm font-medium">
                    ...
                    <div class="flex items-end gap-2">
                        ...
                        <span class="flex-1 border-b border-black border-dotted h-4 pb-0.5 pl-2 font-normal">{{ ayarlar.tarih or '' }}</span>
                    </div>
                </div>
            </header>
                ...
                <div class="soru-blok relative group">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-shrink-0 pt-1">
                            <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-slate-900 text-white font-bold text-sm shadow-sm print:bg-black print:text-white">
                                {{ loop.index }}
                            </span>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mb-4 text-center">
                                <img src="{{ soru.gorsel_yolu }}" alt="Soru görseli" class="max-w-full h-auto max-h-60 rounded-lg border border-slate-200 object-contain mx-auto print:border-none">
                            </div>
                            {% endif %}

                            <div class="flex flex-col sm:flex-row gap-6 {{ 'items-start' if soru.gorsel_konum == 'yanda' else '' }}">
                                <div class="flex-1">
                                    <!-- Soru Metni -->
                                    <div class="text-[15px] text-slate-900 leading-relaxed font-medium mb-3 text-justify whitespace-pre-line">{{ soru.soru_metni }}</div>
                                    ...
                                    <!-- Şıklar -->
                                    {% if soru.secenek_a or soru.secenek_b or soru.secenek_c or soru.secenek_d or soru.secenek_e %}
                                        ...
                                        <div class="mt-4 {{ sik_class }} ml-1">
                                            {% for sik_harfi in ['A', 'B', 'C', 'D', 'E'] %}
                                                ...
                                                <div class="flex items-start gap-2 min-w-[100px] group/sik">
                                                    <span class="flex-shrink-0 w-6 h-6 rounded-full border border-slate-400 flex items-center justify-center text-xs font-bold print:border-black group-hover/sik:border-slate-600 transition-colors">{{ sik_harfi }}</span>
                                                    <span class="text-sm text-slate-900 pt-0.5 leading-snug">{{ soru[sik_attr] }}</span>
                                                </div>
                                                {% endif %}
                                            {% endfor %}
                                        </div>
                                ...
                                <div class="flex-shrink-0 w-full sm:w-1/3 ml-0 sm:ml-4 mb-4 sm:mb-0">
                                     <img src="{{ soru.gorsel_yolu }}" alt="Soru görseli" class="w-full h-auto rounded-lg border border-slate-200 object-contain print:border-none bg-slate-50">
                                </div>
                                {% endif %}
                            </div>
                        </div>
                        ...
                        <div class="hidden print:block absolute top-0 right-0 text-xs font-bold border border-black px-1 rounded">
                            {{ soru.puan }}p
                        </div>
                        {% endif %}
                    </div>
