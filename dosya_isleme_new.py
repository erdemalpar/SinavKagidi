...
def soru_ayikla_gelismis(metin):
    ...
    try:
        ...
        dogru_cevap = 'A'
        dogru_cevap_metni = None
        
        for i in range(len(satirlar) - 1, -1, -1):
            cevap_match = re.search(r'(?:Cevap|Doğru)[:\s]*([A-Ea-e])\)?\s*(.*)$', satirlar[i], re.IGNORECASE)
            if cevap_match:
                dogru_cevap = cevap_match.group(1).upper()
                dogru_cevap_metni = cevap_match.group(2).strip() if cevap_match.group(2).strip() else None
                satirlar.pop(i)
                break
        
        soru_metni = satirlar[0]
        sik_satirlari = satirlar[1:]
        siklar = {}
        harfler = ['A', 'B', 'C', 'D', 'E']
        
        for i, satir in enumerate(sik_satirlari[:5]):
            sik_match = re.match(r'^([A-Ea-e])[).:\s]+(.+)$', satir)
            if sik_match:
                harf = sik_match.group(1).upper()
                siklar[harf] = sik_match.group(2).strip()
            else:
                if i < len(harfler):
                    siklar[harfler[i]] = satir
        
        if dogru_cevap_metni and dogru_cevap not in siklar:
            siklar[dogru_cevap] = dogru_cevap_metni
        
        if len(siklar) < 2:
            print(f"Yeters iz şık: {len(siklar)}")
            return None
